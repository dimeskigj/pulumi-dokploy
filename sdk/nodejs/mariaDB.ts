// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "./utilities";

/**
 * A Dokploy MariaDB database.
 */
export class MariaDB extends pulumi.CustomResource {
    /**
     * Get an existing MariaDB resource's state with the given name, ID, and optional extra
     * properties used to qualify the lookup.
     *
     * @param name The _unique_ name of the resulting resource.
     * @param id The _unique_ provider ID of the resource to lookup.
     * @param opts Optional settings to control the behavior of the CustomResource.
     */
    public static get(name: string, id: pulumi.Input<pulumi.ID>, opts?: pulumi.CustomResourceOptions): MariaDB {
        return new MariaDB(name, undefined as any, { ...opts, id: id });
    }

    /** @internal */
    public static readonly __pulumiType = 'dokploy:index:MariaDB';

    /**
     * Returns true if the given object is an instance of MariaDB.  This is designed to work even
     * when multiple copies of the Pulumi SDK have been loaded into the same process.
     */
    public static isInstance(obj: any): obj is MariaDB {
        if (obj === undefined || obj === null) {
            return false;
        }
        return obj['__pulumiType'] === MariaDB.__pulumiType;
    }

    /**
     * The optional deployed database name.
     */
    declare public readonly appName: pulumi.Output<string | undefined>;
    /**
     * The MariaDB database name.
     */
    declare public readonly databaseName: pulumi.Output<string>;
    /**
     * The MariaDB database password.
     */
    declare public readonly databasePassword: pulumi.Output<string>;
    /**
     * The optional MariaDB root password.
     */
    declare public readonly databaseRootPassword: pulumi.Output<string | undefined>;
    /**
     * The MariaDB database user.
     */
    declare public readonly databaseUser: pulumi.Output<string>;
    /**
     * An optional database description.
     */
    declare public readonly description: pulumi.Output<string | undefined>;
    /**
     * The MariaDB Docker image.
     */
    declare public readonly dockerImage: pulumi.Output<string>;
    /**
     * Environment variables for MariaDB.
     */
    declare public readonly environment: pulumi.Output<string | undefined>;
    /**
     * The target environment ID.
     */
    declare public readonly environmentId: pulumi.Output<string>;
    /**
     * The optional externally exposed port.
     */
    declare public readonly externalPort: pulumi.Output<number | undefined>;
    /**
     * The stable Dokploy MariaDB ID.
     */
    declare public /*out*/ readonly mariadbId: pulumi.Output<string>;
    /**
     * The database resource name.
     */
    declare public readonly name: pulumi.Output<string>;
    /**
     * The optional server ID.
     */
    declare public readonly serverId: pulumi.Output<string | undefined>;
    /**
     * The current MariaDB deployment status.
     */
    declare public /*out*/ readonly status: pulumi.Output<string>;

    /**
     * Create a MariaDB resource with the given unique name, arguments, and options.
     *
     * @param name The _unique_ name of the resource.
     * @param args The arguments to use to populate this resource's properties.
     * @param opts A bag of options that control this resource's behavior.
     */
    constructor(name: string, args: MariaDBArgs, opts?: pulumi.CustomResourceOptions) {
        let resourceInputs: pulumi.Inputs = {};
        opts = opts || {};
        if (!opts.id) {
            if (args?.databaseName === undefined && !opts.urn) {
                throw new Error("Missing required property 'databaseName'");
            }
            if (args?.databasePassword === undefined && !opts.urn) {
                throw new Error("Missing required property 'databasePassword'");
            }
            if (args?.databaseUser === undefined && !opts.urn) {
                throw new Error("Missing required property 'databaseUser'");
            }
            if (args?.environmentId === undefined && !opts.urn) {
                throw new Error("Missing required property 'environmentId'");
            }
            if (args?.name === undefined && !opts.urn) {
                throw new Error("Missing required property 'name'");
            }
            resourceInputs["appName"] = args?.appName;
            resourceInputs["databaseName"] = args?.databaseName;
            resourceInputs["databasePassword"] = args?.databasePassword ? pulumi.secret(args.databasePassword) : undefined;
            resourceInputs["databaseRootPassword"] = args?.databaseRootPassword ? pulumi.secret(args.databaseRootPassword) : undefined;
            resourceInputs["databaseUser"] = args?.databaseUser;
            resourceInputs["description"] = args?.description;
            resourceInputs["dockerImage"] = (args?.dockerImage) ?? "mariadb:11";
            resourceInputs["environment"] = args?.environment ? pulumi.secret(args.environment) : undefined;
            resourceInputs["environmentId"] = args?.environmentId;
            resourceInputs["externalPort"] = args?.externalPort;
            resourceInputs["name"] = args?.name;
            resourceInputs["serverId"] = args?.serverId;
            resourceInputs["mariadbId"] = undefined /*out*/;
            resourceInputs["status"] = undefined /*out*/;
        } else {
            resourceInputs["appName"] = undefined /*out*/;
            resourceInputs["databaseName"] = undefined /*out*/;
            resourceInputs["databasePassword"] = undefined /*out*/;
            resourceInputs["databaseRootPassword"] = undefined /*out*/;
            resourceInputs["databaseUser"] = undefined /*out*/;
            resourceInputs["description"] = undefined /*out*/;
            resourceInputs["dockerImage"] = undefined /*out*/;
            resourceInputs["environment"] = undefined /*out*/;
            resourceInputs["environmentId"] = undefined /*out*/;
            resourceInputs["externalPort"] = undefined /*out*/;
            resourceInputs["mariadbId"] = undefined /*out*/;
            resourceInputs["name"] = undefined /*out*/;
            resourceInputs["serverId"] = undefined /*out*/;
            resourceInputs["status"] = undefined /*out*/;
        }
        opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts);
        const secretOpts = { additionalSecretOutputs: ["databasePassword", "databaseRootPassword", "environment"] };
        opts = pulumi.mergeOptions(opts, secretOpts);
        const replaceOnChanges = { replaceOnChanges: ["environmentId", "serverId"] };
        opts = pulumi.mergeOptions(opts, replaceOnChanges);
        super(MariaDB.__pulumiType, name, resourceInputs, opts);
    }
}

/**
 * The set of arguments for constructing a MariaDB resource.
 */
export interface MariaDBArgs {
    /**
     * The optional deployed database name.
     */
    appName?: pulumi.Input<string | undefined>;
    /**
     * The MariaDB database name.
     */
    databaseName: pulumi.Input<string>;
    /**
     * The MariaDB database password.
     */
    databasePassword: pulumi.Input<string>;
    /**
     * The optional MariaDB root password.
     */
    databaseRootPassword?: pulumi.Input<string | undefined>;
    /**
     * The MariaDB database user.
     */
    databaseUser: pulumi.Input<string>;
    /**
     * An optional database description.
     */
    description?: pulumi.Input<string | undefined>;
    /**
     * The MariaDB Docker image.
     */
    dockerImage?: pulumi.Input<string | undefined>;
    /**
     * Environment variables for MariaDB.
     */
    environment?: pulumi.Input<string | undefined>;
    /**
     * The target environment ID.
     */
    environmentId: pulumi.Input<string>;
    /**
     * The optional externally exposed port.
     */
    externalPort?: pulumi.Input<number | undefined>;
    /**
     * The database resource name.
     */
    name: pulumi.Input<string>;
    /**
     * The optional server ID.
     */
    serverId?: pulumi.Input<string | undefined>;
}
