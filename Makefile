...
gen_examples: codegen install_plugin
	...
	python3 -c 'from pathlib import Path; p=Path("examples/java/src/main/java/generated_program/App.java"); s=p.read_text().replace("com.gjorgjidimeski.dokploy", "dev.codechem.pulumi.dokploy").replace("config.requireObject(\"dokploy:endpoint\", com.pulumi.core.TypeShape.map(String.class, Object.class))", "config.require(\"dokploy:endpoint\")").replace("config.requireObject(\"dokploy:apiKey\", com.pulumi.core.TypeShape.map(String.class, Object.class))", "config.requireSecret(\"dokploy:apiKey\")").replace("config.getSecret(\"registryPassword\").orElse(\"replace-with-a-registry-password\")", "config.getSecret(\"registryPassword\").applyValue(v -> v.orElse(\"replace-with-a-registry-password\"))").replace("config.getSecret(\"databasePassword\").orElse(\"replace-with-a-database-password\")", "config.getSecret(\"databasePassword\").applyValue(v -> v.orElse(\"replace-with-a-database-password\"))").replace("config.getSecret(\"redisPassword\").orElse(\"replace-with-a-redis-password\")", "config.getSecret(\"redisPassword\").applyValue(v -> v.orElse(\"replace-with-a-redis-password\"))"); p.write_text(s)'
...
ci-mgmt:
	...
	python3 scripts/normalize_ci.py
